...
export async function POST(req: NextRequest) {
  try {
    ...
    const userImagePart = fileToGenerativePart(body.imageBase64, body.imageMime);
    ...
    let result = await model.generateContent([promptForKeywords, userImagePart]);
    let keywords = result.response.text().split(',').map(k => k.trim().toLowerCase()); // Convert to lowercase for consistency
    
    if (keywords.length === 0) {
      return NextResponse.json({ foundProduct: null, message: "Sorry, the AI could not identify keywords from the image." });
    }
    ...
    const { data: candidateProducts, error: dbError } = await supabase
      .rpc('search_products_by_tags', { search_tags: keywords });

    if (dbError || !candidateProducts) {
      console.error('Supabase error:', dbError);
      return NextResponse.json({ error: 'Failed to search for products.' }, { status: 500 });
    }
    
    if (candidateProducts.length === 0) {
      return NextResponse.json({ foundProduct: null, message: "Sorry, we couldn't find a matching product in our catalog." });
    }
    ...
    if (candidateProducts.length === 1) {
      ...
      const productOptions = candidateProducts.map(p => `Product Code: ${p.product_code}, Name: ${p.product_name}, Tags: ${p.tags.join(', ')}`).join('\n');
      ...
      const matchedProduct = candidateProducts.find(p => p.product_code === bestProductCode);
